...
class shoppingRamosLocally:
    def __init__(self,driver):
        
        self.driver=driver

    def compraChicharronLocally(self,telefono):
        ...
        if btn_proceedCheckout is not None:
            ...
            self.driver.implicitly_wait(10)
            time.sleep(8)
        pickLocal=self.driver.find_element_by_xpath('//*[@id="navTabsWrapper"]/div/ul/li[2]/a')
        if pickLocal is not None:
            print("Haciendo click en la leyeda de Gobstore para recoger local")
            pickLocal.click()
            self.driver.implicitly_wait(10)
            time.sleep(8)
        pickLocalCheckbox=self.driver.find_element_by_xpath('//*[@id="uiPickUpPoints"]/li/input')
        if pickLocalCheckbox is not None:
            print("Haciendo click en el checkbox de Gobstore para recoger local")
            pickLocalCheckbox.click()
            self.driver.implicitly_wait(10)
            time.sleep(8)
        btn_TDD_TDC = self.driver.find_element_by_xpath('//*[@id="paymentTypeSelector"]/li[1]/a')
        if btn_TDD_TDC is not None:
            print("Eligiendo metodo de pago con tarjeta")
            time.sleep(2)
            btn_TDD_TDC.click()
            self.driver.implicitly_wait(10)
            time.sleep(8)
        ...
        telephone = self.driver.find_element_by_id("contactPhoneNumber")
        if telephone is not None:
            print("Campo de teléfono encontrado")
            telephone.send_keys(telefono)
            time.sleep(3)            
